...
def get_data(df):
    data_tmp = df.filter(lambda x:
                         (x['content']['tid'] == '15cd698f13ec3769aba039bf013f2803') and
                         (x['content']['os'] == 'ios')).map(lambda x: transData(x))
    data = spark.createDataFrame(data_tmp, samplingRatio=1)
    data.persist(StorageLevel.DISK_ONLY)
    data_cnt = data.count()
...
if __name__ == "__main__":
    ...
    data = get_data(df=df)
    data.coalesce(1).write.mode("overwrite").option("header","true").csv(save_path)
