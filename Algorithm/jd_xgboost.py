...
def jd_xgb(data, missing=0, postfix='part1'):
    # img_path = img_save_path.format(postfix)
    print("image path {0}".format(img_path))
    np_train, np_test = split_np(data, 0.7)
    X_train = np_train[:, :-1]
    X_test = np_test[:, :-1]
    y_train = np_train[:, -1]
    y_test = np_test[:, -1]
    ...
    params_path = "{0}/grid_search_params/{1}_{2}.json".format(Analysis_path, inspect.stack()[0][3], postfix)
    ...
    # draw roc on xgboost
    pic_pr_cmp(title="JD Rsik Login PR Curve", file_path=img_path, y_data=cmp_pr)
    pic_pr_cmp_threshold(title="JD Rsik Login Pecision & Recall", file_path=img_path, y_data=cmp_pr)
    pic_roc_cmp(title="JD Rsik Login ROC", file_path=img_path, y_data=cmp_pr)
    ##f1 score
    best_threshold = pic_fbeta_cmp(title="JD Rsik Login Fbeta-Score", file_path=img_path, y_data=cmp_pr, beta=0.1)


def jd_xgb2(missing=0, postfix='part1'):
    print("image path {0}".format(v2_img_path))
    train_data = pd.read_csv(v2_train_data)
    test_data = pd.read_csv(v2_test_data)
    label_cols = train_data.columns.values[label_match(train_data.columns.values)].tolist()
    X_cols = train_data.columns.difference(label_cols)
    X_train = train_data[X_cols]
    X_test = test_data[X_cols]
    y_train = train_data.ix[:, train_data.columns == 'label']
    y_test = test_data.ix[:, test_data.columns == 'label']
...
def jd_xgb3(missing=0, postfix='part1'):
    print("image path {0}".format(v3_img_path))
    np_train = np.load(v3_train_data)
    np_test = np.load(v3_test_data)
    X_train = np_train[:, :-1]
    X_test = np_test[:, :-1]
    y_train = np_train[:, -1]
    y_test = np_test[:, -1]
    ...
    # draw roc on xgboost
    pic_pr_cmp(title="JD Rsik Login PR Curve", file_path=v3_img_path, y_data=cmp_pr)
    pic_pr_cmp_threshold(title="JD Rsik Login Pecision & Recall", file_path=v3_img_path, y_data=cmp_pr)
    pic_roc_cmp(title="JD Rsik Login ROC", file_path=v3_img_path, y_data=cmp_pr)
    ##f1 score
    best_threshold = pic_fbeta_cmp(title="JD Rsik Login Fbeta-Score", file_path=v3_img_path, y_data=cmp_pr, beta=0.1)